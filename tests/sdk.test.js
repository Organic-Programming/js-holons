...
const { transport, identity, serve, grpcclient, holonrpc } = require('../src/index');
const runtimeState = require('../src/_runtime_state');
...
describe('transport', () => {
    it('scheme() extracts transport scheme', () => {
        ...
        assert.equal(transport.scheme('mem://'), 'mem');
    ...
    it('parseURI() parses mem:// aliases', () => {
        const parsedA = transport.parseURI('mem://');
        const parsedB = transport.parseURI('mem');
        assert.equal(parsedA.scheme, 'mem');
        assert.equal(parsedB.scheme, 'mem');
    });
    ...
    it('MemListener supports dial() and accept()', async () => {
        const mem = transport.listen('mem://');
        assert.ok(mem instanceof transport.MemListener);

        const acceptPromise = mem.accept();
        const pair = await mem.dial();
        const serverConn = await acceptPromise;

        const readPromise = new Promise((resolve) => {
            serverConn.once('data', (chunk) => resolve(chunk.toString('utf8')));
        });

        pair.client.write(Buffer.from('hello mem'));
        const got = await readPromise;
        assert.equal(got, 'hello mem');
...
describe('grpcclient', () => {
    ...
    it('dialMem() resolves registered mem endpoint', () => {
        runtimeState.registerMemEndpoint('mem://test', '127.0.0.1:19090');
        const client = grpcclient.dialMem('mem://test', FakeClient);
        assert.equal(client.target, '127.0.0.1:19090');
        runtimeState.unregisterMemEndpoint('mem://test');
    });

    it('dialMem() fails for missing endpoint', () => {
        assert.throws(() => grpcclient.dialMem('mem://missing', FakeClient), /no mem endpoint/);
    });
