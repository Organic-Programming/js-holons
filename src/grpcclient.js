...
function dialMem(memURI, ClientCtor, options = {}) {
    assertClientCtor(ClientCtor);

    const key = normalizeMemURI(memURI || 'mem://');
    const target = resolveMemEndpoint(key);
    if (!target) {
        throw new Error(`no mem endpoint is registered for ${key}`);
    }
...
async function dialURI(uri, ClientCtor, options = {}) {
    ...
    if (s === 'mem') {
        return { client: dialMem(uri, ClientCtor, options), close: async () => {} };
    }
...
function normalizeDialTarget(addressOrURI) {
    ...
    throw new Error(
        `dial() supports tcp:// and unix:// targets. Use dialMem/dialStdio/dialWebSocket for ${parsed.scheme}://.`
    );
}
