...
async function scanDir(root, dir, origin, entriesByKey, orderedKeys) {
    ...
    for (const child of children) {
        ...
        if (!child.isFile() || child.name !== 'holon.yaml') {
            continue;
        }

        try {
            const identity = parseHolon(childPath);
            const manifest = parseManifest(childPath);
            const absDir = path.resolve(path.dirname(childPath));
            const entry = {
...
function parseManifest(filePath) {
    ...
    const build = raw.build && typeof raw.build === 'object' && !Array.isArray(raw.build) ? raw.build : {};
    const artifacts =
        raw.artifacts && typeof raw.artifacts === 'object' && !Array.isArray(raw.artifacts) ? raw.artifacts : {};
    return {
        kind: String(raw.kind || ''),
        build: {
            runner: String(build.runner || ''),
            main: String(build.main || ''),
        },
        artifacts: {
            binary: String(artifacts.binary || ''),
            primary: String(artifacts.primary || ''),
        },
    };
}
